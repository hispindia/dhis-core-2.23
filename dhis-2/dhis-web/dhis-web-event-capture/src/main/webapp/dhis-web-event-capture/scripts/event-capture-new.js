...
{
    if ($.isArray(dhis2.ec.currentOrganisationUnitId))
    {
        return dhis2.ec.currentOrganisationUnitId[0];
    }

    return dhis2.ec.currentOrganisationUnitId;
}
    ...
    return function() {
        return $.ajax( {
            ...
            _.each(_.values( program.organisationUnits), function(o){
                program[o.id] = {name: o.name, id: o.id};
            });
            
            delete program.organisationUnits;
...
function uploadLocalData()
{
    ...
    var dataValues = dhis2.ec.storageManager.getAllDataValues();
    var completeDataSets = dhis2.ec.storageManager.getCompleteDataSets();
    ...
    function pushCompleteDataSets(array)
    {
        ...
        $.ajax({
            ...
            {
                dhis2.ec.storageManager.clearCompleteDataSet(value);
    ...
    (function pushDataValues(array)
    {
        ...
        $.ajax({
            ...
            {
                dhis2.ec.storageManager.clearDataValueJSON(value);
...
function addEventListeners()
{
    ...
    var formType = dhis2.ec.dataSets[dataSetId].type;
    ...
    $('.entryfield').each(function(i)
    {
        ...
        dhis2.ec.currentOrganisationUnitId = split.organisationUnitId;
...
function loadForm()
{
    var dataSetId = dhis2.ec.currentDataSetId;

    dhis2.ec.currentOrganisationUnitId = selection.getSelected()[0];

    if (!dhis2.ec.multiOrganisationUnit && dhis2.ec.storageManager.formExists(dataSetId))
    {
        log('Loading form locally: ' + dataSetId);

        var html = dhis2.ec.storageManager.getForm(dataSetId);

        $('#contentDiv').html(html);

        if (dhis2.ec.dataSets[dataSetId].renderAsTabs) {
            $("#tabs").tabs();
        }
    ...
    else
    {
        ...
        $('#contentDiv').load('loadForm.action',
                {
                    dataSetId: dataSetId,
                    multiOrganisationUnit: dhis2.ec.multiOrganisationUnit ? getCurrentOrganisationUnit() : ''
                },
        function()
        {
            if (!dhis2.ec.multiOrganisationUnit)
            {
                if (dhis2.ec.dataSets[dataSetId].renderAsTabs) {
                    $("#tabs").tabs();
                }
...
{
    if (dhis2.ec.dataElements[dataElementId] != null)
    {
        return dhis2.ec.dataElements[dataElementId];
    }
...
{
    if (dhis2.ec.metaDataIsLoaded == false)
    {
        return false;
    }

    dhis2.ec.currentOrganisationUnitId = orgUnits[0];
    ...
    if (children)
    {
        ...
        if (childrenDataSets && childrenDataSets.length > 0)
        {
            ...
            {
                if (dataSetId == item.id && dhis2.ec.multiOrganisationUnit)
                {
    ...
    if (!dhis2.ec.multiOrganisationUnit && dataSetValid && dataSetId != null) {
        $('#selectedDataSetId').val(dataSetId); // Restore selected data set

        if (dhis2.ec.inputSelected() && dhis2.ec.dataEntryFormIsLoaded) {
            resetSectionFilters();
            showLoader();
            loadDataValues();
        }
    }
    else if (dhis2.ec.multiOrganisationUnit && multiDataSetValid && dataSetId != null) {
        $('#selectedDataSetId').val(dataSetId); // Restore selected data set
        dataSetSelected();
    }
    else {
...
{
    var associationSet = orgUnit !== undefined ? dhis2.ec.organisationUnitAssociationSetMap[orgUnit] : dhis2.ec.organisationUnitAssociationSetMap[getCurrentOrganisationUnit()];
    var orgUnitDataSets = dhis2.ec.dataSetAssociationSets[associationSet];
    ...
    $.safeEach(orgUnitDataSets, function(idx, item)
    {
        ...
        var dataSetName = dhis2.ec.dataSets[dataSetId].name;
...
function getSortedDataSetListForOrgUnits(orgUnits)
{
    ...
    $.safeEach(dataSetList, function(idx, item)
    {
        var formType = dhis2.ec.dataSets[item.id].type;
        var found = false;
...
function dataSetSelected()
{
    ...
    var x = dhis2.ec.currentDataSetId;

    var dataSetId = $('#selectedDataSetId').val();
    var periodId = $('#selectedPeriodId').val();

    var previousDataSetValid = (dhis2.ec.currentDataSetId && dhis2.ec.currentDataSetId != -1);
    var previousPeriodType = previousDataSetValid ? dhis2.ec.dataSets[dhis2.ec.currentDataSetId].periodType : null;

    dhis2.ec.currentDataSetId = dataSetId;

    if (dataSetId && dataSetId != -1)
    {
        var periodType = dhis2.ec.dataSets[dataSetId].periodType;
        var allowFuturePeriods = dhis2.ec.dataSets[dataSetId].allowFuturePeriods;
        var periods = dhis2.ec.periodTypeFactory.get(periodType).generatePeriods(dhis2.ec.currentPeriodOffset);
        periods = dhis2.ec.periodTypeFactory.reverse(periods);

        if (allowFuturePeriods == false)
        {
            periods = dhis2.ec.periodTypeFactory.filterFuturePeriods(periods);
        }

        clearListById('selectedPeriodId');
        clearSectionFilters();

        dhis2.ec.currentCategories = dhis2.ec.getCategories(dataSetId);

        dhis2.ec.multiOrganisationUnit = !!$('#selectedDataSetId :selected').data('multiorg');

        var attributeMarkup = dhis2.ec.getAttributesMarkup();
        ...
        if (dhis2.ec.inputSelected() && previousPeriodType && previousPeriodType == periodType)
        {
            showLoader();
...
function periodSelected()
{
    ...
    if (dhis2.ec.inputSelected())
    {
        showLoader();

        if (dhis2.ec.dataEntryFormIsLoaded)
        {
            loadDataValues();
        }
        else
...
{
    if (dhis2.ec.currentPeriodOffset < 0) // Cannot display future periods
    {
        dhis2.ec.currentPeriodOffset++;
 ...
 */
dhis2.ec.getCategories = function(dataSetId)
{
    var dataSet = dhis2.ec.dataSets[dataSetId];

    if (!dataSet || !dataSet.categoryCombo || dhis2.ec.defaultCategoryCombo === dataSet.categoryCombo) {
        return null;
    }

    var categoryCombo = dhis2.ec.categoryCombos[dataSet.categoryCombo];

    var categories = [];

    $.safeEach(categoryCombo.categories, function(idx, cat) {
        var category = dhis2.ec.categories[cat];
 ...
 */
dhis2.ec.categoriesSelected = function()
{
    if (!dhis2.ec.currentCategories || dhis2.ec.currentCategories.length == 0) {
        return true; // No categories present which can be selected
    }

    var options = dhis2.ec.getCurrentCategoryOptions();

    if (!options || options.length < dhis2.ec.currentCategories.length) {
...
/**
 * Returns attribute category combo identifier. Based on the dhis2.ec.currentDataSetId 
 * global variable. Returns null if there is no current data set or if current 
 * data set has the default category combo.
 */
dhis2.ec.getCurrentCategoryCombo = function()
{
    var dataSet = dhis2.ec.dataSets[dhis2.ec.currentDataSetId];
 ...
 * Based on the dhis2.ec.currentCategories global variable. Returns null if there 
 * are no current categories.
 */
dhis2.ec.getCurrentCategoryOptions = function()
{
    if (!dhis2.ec.currentCategories || dhis2.ec.currentCategories.length == 0) {
        return null;
    }
    ...
    $.safeEach(dhis2.ec.currentCategories, function(idx, category) {
 ...
 */
dhis2.ec.getCurrentCategoryOptionsQueryValue = function()
{
    if (!dhis2.ec.getCurrentCategoryOptions()) {
        return null;
    }

    var value = '';

    $.safeEach(dhis2.ec.getCurrentCategoryOptions(), function(idx, option) {
        value += option + ';';
    });
 ...
 */
dhis2.ec.getAttributesMarkup = function()
{
    var html = '';
    ...
    $.safeEach(dhis2.ec.currentCategories, function(idx, category) {
        ...
        html += '<select id="category-' + category.id + '" class="selectionBoxSelect" onchange="dhis2.ec.attributeSelected(\'' + category.id + '\')">';
 ...
 */
dhis2.ec.clearAttributes = function()
{
 ...
 */
dhis2.ec.attributeSelected = function(categoryId)
{
    if (dhis2.ec.inputSelected()) {
        showLoader();
 ...
 */
dhis2.ec.inputSelected = function()
{
    var dataSetId = $('#selectedDataSetId').val();
    var periodId = $('#selectedPeriodId').val();

    if (
            dhis2.ec.currentOrganisationUnitId &&
            dataSetId && dataSetId != -1 &&
            periodId && periodId != -1 &&
            dhis2.ec.categoriesSelected()) {
        return true;
    }
...
function getAndInsertDataValues()
{
    ...
    var params = {
        ...
        multiOrganisationUnit: dhis2.ec.multiOrganisationUnit
    };

    var cc = dhis2.ec.getCurrentCategoryCombo();
    var cp = dhis2.ec.getCurrentCategoryOptionsQueryValue();
...
{
    var dataValues = dhis2.ec.storageManager.getDataValuesInForm(params);
    var complete = dhis2.ec.storageManager.hasCompleteDataSet(params);
...
function insertDataValues(json)
{
    ...
    dhis2.ec.currentMinMaxValueMap = []; // Reset
    ...
    if (!json.locked)
    {
        $.safeEach(json.minMaxDataElements, function(i, value)
        {
            ...
            dhis2.ec.currentMinMaxValueMap[minId] = value.min;
            dhis2.ec.currentMinMaxValueMap[maxId] = value.max;
        });
    }
    ...
    if (json.complete && !json.locked)
    {
        ...
        if (json.storedBy)
        {
            ...
            dhis2.ec.currentCompletedByUser = json.storedBy;
        }
    }
    else
    {
...
function displayEntryFormCompleted()
{
    ...
    dhis2.ec.dataEntryFormIsLoaded = true;
    hideLoader();
...
function registerCompleteDataSet()
{
    ...
    validate(true, function() {
        var params = dhis2.ec.storageManager.getCurrentCompleteDataSetParams();
        ...
        dhis2.ec.storageManager.saveCompleteDataSet(params);
        ...
        $.ajax({
            ...
            success: function(data, textStatus, xhr)
            {
                ...
                dhis2.ec.storageManager.clearCompleteDataSet(params);
            },
            error: function(xhr, textStatus, errorThrown)
            {
                if (409 == xhr.status) // Invalid value or locked
...
{
    if (dhis2.ec.currentCompletedByUser)
    {
        var url = '../dhis-web-commons-ajax-json/getUser.action';

        $.getJSON(url, {username: dhis2.ec.currentCompletedByUser}, function(json)
        {
...
function validate(ignoreSuccessfulValidation, successCallback)
{
    ...
    var validCompleteOnly = dhis2.ec.dataSets[dhis2.ec.currentDataSetId].validCompleteOnly;
...
{
    var fieldCombinationRequired = dhis2.ec.dataSets[dhis2.ec.currentDataSetId].fieldCombinationRequired;
...
function updateForms()
{
    ...
    DAO.store.open().done(function() {
        dhis2.ec.loadOptionSets();
    });
}

function purgeLocalForms()
{
    var formIds = dhis2.ec.storageManager.getAllForms();

    $.safeEach(formIds, function(idx, item)
    {
        if (dhis2.ec.dataSets[item] == null)
        {
            dhis2.ec.storageManager.deleteForm(item);
            dhis2.ec.storageManager.deleteFormVersion(item);
...
{
    var formIds = dhis2.ec.storageManager.getAllForms();
    var formVersions = dhis2.ec.storageManager.getAllFormVersions();

    $.safeEach(formIds, function(idx, item)
    {
        var remoteVersion = dhis2.ec.dataSets[item].version;
        var localVersion = formVersions[item];

        if (remoteVersion == null || localVersion == null || remoteVersion != localVersion)
        {
            dhis2.ec.storageManager.downloadForm(item, remoteVersion);
        }
    });
}
...
{
    $.safeEach(dhis2.ec.dataSets, function(idx, item)
    {
        var remoteVersion = item.version;

        if (!dhis2.ec.storageManager.formExists(idx) && !item.skipOffline)
        {
            dhis2.ec.storageManager.downloadForm(idx, remoteVersion);
        }
    });
}
 ...
 * set from server.
 */
dhis2.ec.searchOptionSet = function(uid, query, success)
{
    if (window.DAO !== undefined && window.DAO.store !== undefined) {
        DAO.store.get('optionSets', uid).done(function(obj) {
            if (obj) {
                ...
                dhis2.ec.getOptions(uid, query, success);
            }
        });
    }
    else {
 ...
 */
dhis2.ec.getOptions = function(uid, query, success)
{
    $.ajax({
 ...
 */
dhis2.ec.loadOptionSets = function()
{
    var options = _.values(dhis2.ec.optionSets); // Array of objects with uid and v
    var uids = [];
 ...
 */
dhis2.ec.insertOptionSets = function()
{
    $('.entryoptionset').each(function(idx, item) {
        ...
        dhis2.ec.autocompleteOptionSetField(item, dhis2.ec.optionSets[optionSetKey].uid);
    });
};
 ...
 */
dhis2.ec.autocompleteOptionSetField = function(idField, optionSetUid)
{
    ...
    input.autocomplete({
        ...
        source: function(request, response) {
            dhis2.ec.searchOptionSet(optionSetUid, input.val(), response);
        },
